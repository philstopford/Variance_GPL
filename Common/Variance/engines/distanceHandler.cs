...
internal class DistanceHandler
{
    ...
    private void distanceHandlerLogic(Paths aPaths, Paths bPaths, EntropySettings simulationSettings, bool runThreaded)
    {
        resultPaths = new Paths();
        ...
        {
            Path tempPath = new() {new IntPoint(0, 0)};
    ...
    private spaceResult fastKDTree(Paths aPaths, Paths bPaths, EntropySettings simulationSettings)
    {
    ...
    private void overlap(Paths aPaths, Paths bPaths, EntropySettings simulationSettings, bool runThreaded)
    {
        ...
        foreach (Path layerAPath in aPaths)
        {
            foreach (Path layerBPath in bPaths)
            {
                ...
                if (simulationSettings.getValue(EntropySettings.properties_i.subMode) == (int)CommonVars.spacingCalcModes.spacing || simulationSettings.getValue(EntropySettings.properties_i.subMode) == (int)CommonVars.spacingCalcModes.spacingOld) // spacing
                {
                    ...
                    AreaHandler aH = new(new Paths { layerAPath }, new Paths { layerBPath }, maySimplify: false, perPoly: false, scaleFactorForPointF: 1.0);
    ...
    private spaceResult doPartialOverlap(Paths overlapShape, Path aPath, Path bPath, EntropySettings simulationSettings, bool runThreaded)
    {
        spaceResult result = new();
        ...
        KDTree<GeoLibPointF> aTree = new(2, aPathCount);
        KDTree<GeoLibPointF> bTree = new(2, bPathCount);
        ...
        try
        {
            ...
            for (int poly = 0; poly < oCount; poly++)
            {
                ...
                foreach (spaceResult tResult in aOverlapEdge.SelectMany(t => bOverlapEdge.Select(t1 => overlapAssess(simulationSettings, overlapShape[poly], t, t1, aPath, bPath, runThreaded)).Where(tResult => result.resultPaths.Count == 0 || tResult.distance > result.distance)))
                {
    ...
    private spaceResult overlapAssess(EntropySettings simulationSettings, Path overlapPoly, Path aOverlapEdge, Path bOverlapEdge, Path aPath, Path bPath, bool runThreaded)
    {
        spaceResult result = new();
        ...
        */
        ...
        IntPoint shortestPathBeforeStartPoint = new(0, 0);
        IntPoint shortestPathAfterEndPoint = new(0, 0);
        ...
        RayCast rc = new(extractedPath, overlapPoly, CentralProperties.scaleFactorForOperation * CentralProperties.scaleFactorForOperation, runThreaded, invert, 0, true, false, shortestPathBeforeStartPoint, shortestPathAfterEndPoint);
