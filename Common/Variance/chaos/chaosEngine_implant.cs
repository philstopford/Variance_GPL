...
namespace Variance
{
    ...
    class ChaosEngine_implant
    {
        ...
        // Output geometry.
        GeoLibPointF[] geom, bgGeom;
        public GeoLibPointF[] getGeom()
        {
            return pGetGeom();
        }

        GeoLibPointF[] pGetGeom()
        {
            return geom;
        }
        ...
        GeoLibPointF[] pGetBGGeom()
        {
            return bgGeom;
        }

        GeoLibPointF[] shadow, minShadow, maxShadow;
        public GeoLibPointF[] getShadow()
        {
            return pGetShadow();
        }

        GeoLibPointF[] pGetShadow()
        {
            return shadow;
        }
        ...
        void pChaosEngine_implant(ChaosSettings_implant chaosSettings, EntropySettings entropySettings, EntropyImplantSettings implantCalcSettings, double angularTolerance)
        {
            ...
            try
            {
                List<EntropyLayerSettings> implant_MLS = new List<EntropyLayerSettings>();
                EntropyLayerSettings mls = new EntropyLayerSettings();
                mls.setInt(EntropyLayerSettings.properties_i.edgeSlide, 0); // we don't want the edge slide in this situation.
                mls.setInt(EntropyLayerSettings.properties_i.shapeIndex, (Int32)CommonVars.shapeNames.rect);
                ...
                mls.setInt(EntropyLayerSettings.properties_i.posIndex, (Int32)CommonVars.subShapeLocations.C);
                ...
                if (reIndexStart != 0)
                {
                    Int32 index = 0; // just to track our position in the points array.
                    for (Int32 pt = reIndexStart; pt < solution[0].Count; pt++)
                    {
                ...
                for (int pt = points.Length - 3; pt > 1; pt--)
                {
                    ...
                    if (chaosSettings.isPreview())
                    {
                        ...
                        if ((nomPtIndex != -1) && (maxPtIndex == -1) && (angle + 90.0f >= Convert.ToDouble(tiltAngle) * Math.Cos(Utils.toRadians(implantCalcSettings.getDouble(EntropyImplantSettings.properties_d.twist)))))
                        {
                            maxPtIndex = pt;
                            maxXIntercept = -c_ / m;
                            maxYIntercept = c_;
