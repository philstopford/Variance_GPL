...
public class EntropyShape
{
    ...
    private double globalBias_Sides, globalBias_Tips;

    private MyVertex[] Vertex; // container for our extended point properties information.
    private MyRound[] round1; // container for the rounding data.
    private bool[] tips; // large array to hold tip information.
    ...
    private List<GeoLibPointF> preFlight(List<GeoLibPointF> mcPoints, EntropyLayerSettings entropyLayerSettings)
    {
    ...
    private List<GeoLibPointF> makeShape(bool returnEarly, bool cornerCheck, EntropySettings entropySettings, List<EntropyLayerSettings> entropyLayerSettingsList, int settingsIndex, bool doPASearch, bool previewMode, ChaosSettings chaosSettings, ShapeLibrary shape)
    {
        ...
        {
            mcPoints.Clear();
            mcPoints.AddRange(Vertex.Select(t => new GeoLibPointF(t.X, t.Y)));
            return mcPoints;
        }
    ...
    private void makeEntropyShape(EntropySettings entropySettings, List<EntropyLayerSettings> entropyLayerSettingsList, int settingsIndex, bool doPASearch, bool previewMode, ChaosSettings chaosSettings, ShapeLibrary shape = null, GeoLibPointF pivot_ = null)
    {
        ...
        if (mcPoints.Count > 1)
        {
            ...
            List<GeoLibPointF> newPoints = new();
        ...
        if (entropyLayerSettingsList[settingsIndex].getInt(EntropyLayerSettings.properties_i.perPoly) == 1 && entropyLayerSettingsList[settingsIndex].getInt(EntropyLayerSettings.properties_i.shapeIndex) == (int)CommonVars.shapeNames.GEOCORE)
        {
        ...
        for (int i = 0; i < mcPoints.Count; i++)
        {
            points[i] = mcPoints[i];
        }
