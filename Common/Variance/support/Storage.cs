...
namespace Variance
{
    public class Storage
    {
        ...
        void modifyLayerRefs_forLayerChange(ref EntropyLayerSettings readSettings, Int32 compatibilityLayerOffset)
        {
            if (compatibilityLayerOffset == 6) // v1.x-2.1 file
            {
            ...
            if (compatibilityLayerOffset == 4) // v2.2 file
            {
        ...
        bool pStoreSimulationSettings(string filename, EntropySettings simulationSettings, EntropySettings_nonSim simulationSettings_nonSim, List<EntropyLayerSettings> listOfSettings, EntropySettings implantSimSettings, EntropySettings_nonSim implantSettings_nonSim, EntropyImplantSettings implantSettings_, NonSimulationSettings nonSimulationSettings_)
        {
        ...
        string pLoadSimulationSettings(string currentVersion, string filename, EntropySettings simulationSettings, EntropySettings_nonSim simulationSettings_nonSim, List<EntropyLayerSettings> listOfSettings, EntropySettings implantSimSettings, EntropySettings_nonSim implantSettings_nonSim, EntropyImplantSettings implantSettings_, NonSimulationSettings nonSimulationSettings_)
        {
            ...
            for (int layer = 0; layer < CentralProperties.maxLayersForMC; layer++)
            {
                ...
                try
                {
                    ...
                    {
                        if (readSettings.getInt(EntropyLayerSettings.properties_i.shapeIndex) == (Int32)CommonVars.shapeNames.Sshape)
                        {
                            readSettings.setInt(EntropyLayerSettings.properties_i.shapeIndex, (Int32)CommonVars.shapeNames.GEOCORE); //increment to map to the GDS setting in the 1.9 version.
                        }
                    }
                }
                catch (Exception)
                ...
                Int32 compatibilityLayerOffset = 0; // to bump layer 3 to layer 5; layer 4 to layer 6.
                if ((Convert.ToInt32(tokenVersion[0]) == 1) ||
                    ((Convert.ToInt32(tokenVersion[0]) == 2) && (Convert.ToInt32(tokenVersion[1]) < 1)))
                {
                    compatibilityLayerOffset = 6;
                }
                else
                {
                    if (Convert.ToInt32(tokenVersion[0]) == 2)
                    {
                ...
                if (compatibilityLayerOffset != 0)
                {
                    ...
                    if (((compatibilityLayerOffset == 6) && ((layer == 2) || (layer == 3))) ||
                        ((compatibilityLayerOffset == 4) && ((layer == 4) || (layer == 5) || (layer == 6) || (layer == 7))))
                    {
                        // Full layer remap
            ...
            try
            {
                ...
                catch (Exception)
                {
                    try
                    {
                        ...
                        simulationSettings.setValue(EntropySettings.properties_i.cSeg, (Int32)(90.0f / angleStep));
                    }
                    catch (Exception)
                    {
