...
public static class UtilityFuncs
{
    ...
    private static double pGetGaussianRandomNumber3(EntropySettings settings)
    {
        double value;
    ...
    private static double pGetRandomDouble(EntropySettings settings)
    {
        double value;
        switch (settings.getValue(EntropySettings.properties_i.rngType))
        {
    ...
    private static void extractValuesFromSummaryFile(DOEResults doeResults, string[] DOETokens, string filename)
    {
        ...
        int column = Convert.ToInt32(tempArray[1]);
        tempArray = DOETokens[2].Split('w');
        int row = Convert.ToInt32(tempArray[1]);
        ...
        while ((line = file.ReadLine()) != null)
        {
            ...
            else
            {
                if (!eqtnFound)
                {
                    const string searchString = "Equation";
    ...
    private static string pGenerateDOESummaryFile(DOEResults doeResults, string path)
    {
        bool error = false;
        string summaryFile = "";
        try
    ...
    private static string pSummarizeDOEResults(string path, string[] summaryFilesFound)
    {
        ...
        DOEResults doeResults = new(DOETokens[0]);
